...
public class Deck {
	 ...
	 * 
	 * @param color
	 *            color of the back of the cards. Must be either Color.BLUE or
	 *            Color.RED.
	 */
	public Deck(Color color) {
		...
		try {
			if (color == null) {
				...
				backImage = ImageIO.read(Card.class
						.getResource("images/back-red-75-1.png"));
			} else if (color.equals(Color.BLUE)) {
				backImage = ImageIO.read(Card.class
						.getResource("images/back-blue-75-1.png"));
			} else {
				throw new IllegalArgumentException(
						"Argument must either Color.RED or Color.BLUE.");
			}
	 ...
	 * 
	 * @param url
	 *            The url of the image file containing the back image of the
	 *            cards. This image should preferably have dimensions width = 75
	 *            and height = 107; otherwise it is scaled to those dimensions,
	 *            and the quality may be poor.
	 */
	public Deck(URL url) {
		Image backImage = null;
		if (url != null) {
			try {
				...
				Logger.getLogger(Deck.class.getName()).log(Level.SEVERE, null,
						ex);
			}
		}
		if (backImage != null
			...
			if (backImage == null) {
				initialize(new BufferedImage(cardWidth, cardHeight,
						BufferedImage.TYPE_INT_ARGB));
			} else {
				initialize(backImage.getScaledInstance(cardWidth, cardHeight,
						Image.SCALE_SMOOTH));
			}
		}
	}
	...
	/**
	 * Gets the next Card from this Deck of cards. The Card that is returned is
	 * random but different from any previously returned Card since the Deck was
	 * last shuffled.
	 * 
	...
	public Card getCard(Card.Suit suit, int number) {
		if (number < 1 || 13 < number) {
			throw new IllegalArgumentException(
					"number must be between 1 and 13.");
		}
		int index = 0;
